...
// 4 frames: pectoral fins cycle open→half→closed→half
// Body is elongated (30 cols × 7 rows), tail fan large
const KOI_R_FRAMES: &[&[&str]] = &[
    // Frame 0: fins spread wide
    &[
        r"                       (---)  ",
        r"       .##~~###~~##. .(-----).",
        r"    {:.#~~#@#~~##~~###(-----) ",
...
fn draw_koi(
    ...
    let frames = if facing_right { KOI_R_FRAMES } else { KOI_L_FRAMES };
    let frame_idx = ((t * 3.0) as usize) % frames.len(); // fins cycle at 3fps
